...
BOWTIE1_VERSION = check_output_decoded('''bowtie --version 2>/dev/null | head -n1''', shell=True).strip()
BOWTIE2_VERSION = check_output_decoded('''bowtie2 --version 2>/dev/null | head -n1 | perl -lane 'print "bowtie2 $1" if m/(version .*)/' ''', shell=True).strip()
TOPHAT2_VERSION = check_output_decoded('''tophat2 --version 2>/dev/null''', shell=True).strip()
HISAT2_VERSION = check_output_decoded('''hisat2 --version 2>/dev/null | head -n1 | perl -lane 'print "hisat2 $1" if m/(version .*)/' ''', shell=True).strip()
...
STAR_VERSION = check_output_decoded('''STAR --version 2>/dev/null''', shell=True).strip()
