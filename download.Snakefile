...
rule get_hg38_analysisSet_twoBit:
    input: static(HTTP.remote('hgdownload.cse.ucsc.edu/goldenpath/hg38/bigZips/analysisSet/hg38.analysisSet.2bit', insecure=True))
...
rule get_hisat2_index_tar:
    input: static(FTP.remote('ftp.ccb.jhu.edu/pub/infphilo/hisat2/data/grch38_snp_tran.tar.gz'))
    output: 'grch38_snp_tran.tar.gz'
    shell: 'mv {input:q} {output:q}'

rule get_gencode_annotation_gff:
    input: static(FTP.remote('ftp.sanger.ac.uk/pub/gencode/Gencode_human/release_{release}/gencode.v{release}.chr_patch_hapl_scaff.annotation.gff3.gz'))
